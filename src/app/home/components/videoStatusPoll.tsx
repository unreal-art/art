...
export default function VideoStatusPoll({ postId, jobId, onVideoReady }: VideoStatusPollProps) {
  const [status, setStatus] = useState<VideoStatus | null>(null);
  const [isPolling, setIsPolling] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [retryCount, setRetryCount] = useState(0);

  console.log(`VideoStatusPoll rendered with postId: ${postId}, jobId: ${jobId}, isPolling: ${isPolling}`);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 404 && retryCount < 10) {
            console.log(`Job ${jobId} not found yet, retrying... (${retryCount + 1}/10)`);
          ...
          if (retryCount < 5) {
            console.log(`API error ${response.status}, retrying... (${retryCount + 1}/5)`);
            setRetryCount(prev => prev + 1);
            return;
          }
          
          throw new Error(`Status check failed after ${retryCount} retries: ${response.statusText}`);
        }
        ...
        if (retryCount >= 10) {
          setError(err instanceof Error ? err.message : "Failed to check video status after multiple retries");
          setIsPolling(false);
        } else {
          setRetryCount(prev => prev + 1);
        }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-primary-13 border border-primary-11 rounded-lg">
        ...
        <h3 className="text-lg font-semibold text-primary-2 mb-2">Video Generation Failed</h3>
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-primary-13 border border-primary-11 rounded-lg">
        ...
        <h3 className="text-lg font-semibold text-primary-2 mb-2">
          {status?.status === "processing" ? "Generating Video..." : "Video In Queue..."}
        </h3>
        <p className="text-primary-4 text-center mb-2">
          Your video is being generated. This may take sometime.
        </p>
        {status && (
          <div className="text-sm text-primary-6 space-y-1">
            <p>Status: <span className="font-medium capitalize text-primary-3">{status.status}</span></p>
            <p>Job ID: <span className="font-mono text-xs text-primary-7">{status.jobId}</span></p>
            {status.updatedAt && (
              <p>Last updated: <span className="text-primary-5">{new Date(status.updatedAt).toLocaleTimeString()}</span></p>
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-primary-13 border border-primary-11 rounded-lg">
      <div className="animate-pulse rounded-lg h-12 w-12 bg-primary-5 mb-4"></div>
      <h3 className="text-lg font-semibold text-primary-2 mb-2">Loading Video Status...</h3>
